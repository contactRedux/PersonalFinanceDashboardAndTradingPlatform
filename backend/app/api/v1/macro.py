...
async def get_macro_indicators(_: dict = CurrentUser):
    return {
...
async def get_yield_curve(_: dict = CurrentUser):
    return {"curve": [], "note": "Yield curve in ST-10"}


@router.get("/fred/{series_id}")
async def get_fred_series(series_id: str, _: dict = CurrentUser):
